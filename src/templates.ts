...
export interface TemplateDef {
  description?: string;
  // Used only when the user supplies no free-text question (see ask()).
  defaultQuestion?: string;
  // The intro block. May reference the supported variables: {{repo}}, {{thread}}, {{date}}.
  body: string;
}
...
export function renderTemplate(body: string, vars: Record<string, string>): string {
  ...
  const rendered = body.replace(PLACEHOLDER_RE, (match, raw: string) => {
    const key = raw.trim();
 ...
 */
export function parseTemplateFile(text: string): ParsedTemplate {
  ...
  for (let i = 1; i < close; i++) {
    ...
    if (KNOWN_FRONTMATTER_KEYS.has(key)) frontmatter[key] = value;
  }
...
export const BUILTIN_TEMPLATES: Record<string, TemplateDef> = {
  review: {
...
export const DEFAULT_TEMPLATE = "review";
